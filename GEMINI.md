...
`.gemini/skills/manim-tts-narration-author/SKILL.md`

The Director must NOT open or read any other skill file. Specialist
skills are at `.gemini/skills/<slug>/SKILL.md` but they are for the
...
After heavy render runs, `.build/` can grow large. Keep `.build`,
`.vendor`, and `.venv` excluded from editor indexing and search.
...
- `.build`, `.venv`, `.gemini/skills/` are gitignored.
